	...
	if (ret == RETURNVALUE_NEEDEXCHANGE) {
		...
		if (ret == RETURNVALUE_NOERROR) {
			...
			if (toCylinder->queryRemove(*toItem, toItem->getItemCount(), flags) == RETURNVALUE_NOERROR) {
				...
				// Adicionando a declaração de actorPlayer
				Player* actorPlayer = (actor && actor->getPlayer()) ? actor->getPlayer() : nullptr;
...
void Game::playerEquipItem(uint32_t playerId, uint16_t spriteId)
{
	...
	if (slotItem && slotItem->getID() == it.id && (!it.stackable || slotItem->getItemCount() == 100 || !equipItem)) {
		...
		ret = internalMoveItem(slotItem->getParent(), player, CONST_SLOT_WHEREEVER, slotItem, slotItem->getItemCount(), nullptr, 0, player, nullptr, &fromPos, &toPos);
	} else if (equipItem) {
		if (slotItem) {
		...
		ret = internalMoveItem(equipItem->getParent(), player, slot, equipItem, equipItem->getItemCount(), nullptr, 0, player, nullptr, &fromPos, &toPos);
	}
...
void Game::updateCreaturesPath(size_t index)
{
	g_scheduler.addEvent(createSchedulerTask(EVENT_CREATURE_PATH_INTERVAL,
		[this, index]() { updateCreaturesPath((index + 1) % EVENT_CREATURECOUNT); }));
...
bool Game::gameIsDay()
{
	int worldTime = getWorldTime(); // Chama a função para obter o tempo atual do mundo
	bool isDay = false; // Inicializa a variável isDay

	if (worldTime >= ((6 * 60) + 30) && worldTime <= ((17 * 60) + 30))
	{
		isDay = true;
	}
...
void Game::notifySpectators(const Creature* creature)
{
	// send to clients
