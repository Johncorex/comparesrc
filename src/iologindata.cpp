...
Account IOLoginData::loadAccount(uint32_t accno)
{
	...
	std::ostringstream query;
	query << "SELECT `id`, `coins`, `tournamentBalance`, `proxy_id`, `name`, `password`, `type`, `premium_ends_at` FROM `accounts` WHERE `id` = " << accno;
...
bool IOLoginData::loginserverAuthentication(const std::string& name, const std::string& password, Account& account)
{
	...
	std::ostringstream query;
	query << "SELECT `id`, `name`, `email`, `password`, `secret`, `type`, `proxy_id`, `premium_ends_at` FROM `accounts` WHERE `name` = " << db.escapeString(name);
	DBResult_ptr result = db.storeQuery(query.str());
	if (!result) {
		return false;
	}
...
bool IOLoginData::loginserverAuthenticationEmail(const std::string& name, const std::string& password, Account& account)
{
	...
	std::ostringstream query;
	query << "SELECT `id`, `name`, `email`, `password`, `secret`, `type`, `premium_ends_at` FROM `accounts` WHERE `email` = " << db.escapeString(name);
	DBResult_ptr result = db.storeQuery(query.str());
	if (!result) {
		return false;
	}
...
bool IOLoginData::preloadPlayer(Player* player, const std::string& name) {
	...
	std::ostringstream query;
	query << "SELECT p.`id`, p.`account_id`, p.`group_id`, p.`deletion`, "
		<< "(SELECT `type` FROM `accounts` WHERE `accounts`.`id` = p.`account_id`) AS `account_type`, "
		<< "(SELECT `coins` FROM `accounts` WHERE `accounts`.`id` = p.`account_id`) AS `coinbalance`, "
		<< "(SELECT `tournamentBalance` FROM `accounts` WHERE `accounts`.`id` = p.`account_id`) AS `tournamentBalance`, "
		<< "a.`premium_ends_at` "
		<< "FROM `players` AS p "
		<< "JOIN `accounts` AS a ON p.`account_id` = a.`id` "
		<< "WHERE p.`name` = " << db.escapeString(name);
...
bool IOLoginData::loadPlayerById(Player* player, uint32_t id)
{
	...
	std::ostringstream query;
	query << "SELECT `id`, `name`, `account_id`, `group_id`, `sex`, `vocation`, `experience`, `level`, `maglevel`, `health`, `healthmax`, `blessings1`, `blessings2`, `blessings3`, `blessings4`, `blessings5`, `blessings6`, `blessings7`, `blessings8`, `mana`, `manamax`, `manaspent`, `soul`, `lookbody`, `lookfeet`, `lookmount`, `lookhead`, `looklegs`, `looktype`, `lookaddons`, `posx`, `posy`, `posz`, `cap`, `lastlogin`, `lastlogout`, `lastip`, `conditions`, `skulltime`, `skull`, `town_id`, `balance`, `bonusrerollcount`, `offlinetraining_time`, `offlinetraining_skill`, `stamina`, `skill_fist`, `skill_fist_tries`, `skill_club`, `skill_club_tries`, `skill_sword`, `skill_sword_tries`, `skill_axe`, `skill_axe_tries`, `skill_dist`, `skill_dist_tries`, `skill_shielding`, `skill_shielding_tries`, `skill_fishing`, `skill_fishing_tries`, `xpboost_value`, `xpboost_stamina`, `instantrewardtokens`, `critical`, `charmpoints`, `direction`, `quick_loot_fallback`, `charmExpansion`, `bestiarykills`, `bestiaryTracker`, `charms`, `autoloot` FROM `players` WHERE `id` = " << id;
...
bool IOLoginData::loadPlayerByName(Player* player, const std::string& name)
{
	...
	std::ostringstream query;
	query << "SELECT `id`, `name`, `account_id`, `group_id`, `sex`, `vocation`, `experience`, `level`, `maglevel`, `health`, `healthmax`, `blessings1`, `blessings2`, `blessings3`, `blessings4`, `blessings5`, `blessings6`, `blessings7`, `blessings8`, `mana`, `manamax`, `manaspent`, `soul`, `lookbody`, `lookfeet`, `lookmount`, `lookhead`, `looklegs`, `looktype`, `lookaddons`, `posx`, `posy`, `posz`, `cap`, `lastlogin`, `lastlogout`, `lastip`, `conditions`, `skulltime`, `skull`, `town_id`, `balance`, `bonusrerollcount`, `offlinetraining_time`, `offlinetraining_skill`, `stamina`, `skill_fist`, `skill_fist_tries`, `skill_club`, `skill_club_tries`, `skill_sword`, `skill_sword_tries`, `skill_axe`, `skill_axe_tries`, `skill_dist`, `skill_dist_tries`, `skill_shielding`, `skill_shielding_tries`, `skill_fishing`, `skill_fishing_tries`, `xpboost_value`, `xpboost_stamina`, `instantrewardtokens`, `critical`, `charmpoints`, `direction`, `quick_loot_fallback`, `charmExpansion`, `bestiarykills`, `bestiaryTracker`, `charms`, `autoloot` FROM `players` WHERE `name` = " << db.escapeString(name);
	return loadPlayer(player, db.storeQuery(query.str()));
}

GuildWarVector IOLoginData::getWarList(uint32_t guildId) {
	DBResult_ptr result = Database::getInstance().storeQuery(fmt::format("SELECT `guild1`, `guild2` FROM `guild_wars` WHERE (`guild1` = {} OR `guild2` = {}) AND `ended` = 0 AND `status` = 1", guildId, guildId));
	if (!result) {
		return {};
	}
...
bool IOLoginData::loadPlayer(Player* player, DBResult_ptr result)
{
	...
	static constexpr size_t size = sizeof(skillNames) / sizeof(skillNames[0]);
...
bool IOLoginData::hasBiddedOnHouse(uint32_t guid)
{
	...
	return db.storeQuery(query.str()).get();
}
