...
FILELOADER_ERRORS Items::loadFromOtb(const std::string& file)
{
	...
	if (majorVersion == 0xFFFFFFFF) {
		...
		return ERROR_INVALID_FORMAT;
	} else if (minorVersion < CLIENT_VERSION_1281) {
		...
		while (stream.read<uint8_t>(attrib)) {
			...
			switch (attrib) {
				...
				case ITEM_ATTR_CLASSIFICATION: {
					if (!stream.skip(1)) {
						return ERROR_SEEK_ERROR; //Returning a value from the FILELOADER_ERRORS enum
					}
					break;
				}
...
void Items::parseItemNode(const pugi::xml_node& itemNode, uint16_t id)
{
	...
	for (auto attributeNode : itemNode.children()) {
		...
		if (tmpStrValue == "type") {
			...
			it.getAbilities().specialSkills[SPECIALSKILL_LIFELEECHCHANCE] = pugi::cast<int32_t>(valueAttribute.value());
		} else if (tmpStrValue == "lifeleechamount") {
			it.getAbilities().specialSkills[SPECIALSKILL_LIFELEECHAMOUNT] = pugi::cast<int32_t>(valueAttribute.value());
		} else if (tmpStrValue == "manaleechchance") {
			it.getAbilities().specialSkills[SPECIALSKILL_MANALEECHCHANCE] = pugi::cast<int32_t>(valueAttribute.value());
		} else if (tmpStrValue == "manaleechamount") {
			it.getAbilities().specialSkills[SPECIALSKILL_MANALEECHAMOUNT] = pugi::cast<int32_t>(valueAttribute.value());
		} else if (tmpStrValue == "coloredhex") {
			it.coloredHex = valueAttribute.as_string(); 
