...
bool InstantSpell::canThrowSpell(const Creature* creature, const Creature* target) const {
	...
	if (range == -1) {
		if (!g_game.canThrowObjectTo(fromPos, toPos, checkLineOfSight, Map::maxClientViewportX - 1, Map::maxClientViewportY - 1)) {
			return false;
		}
	}
	else {
		if (!g_game.canThrowObjectTo(fromPos, toPos, checkLineOfSight, range, range)) {
			return false;
		}
	}
