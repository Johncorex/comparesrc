...
void Player::onEquipImbueItem(Imbuement* imbuement)
{
	...
	for (int32_t ss = SPECIALSKILL_CRITICALHITCHANCE; ss <= SPECIALSKILL_LAST; ++ss) {
		if (imbuement->specialSkills[ss]) {
			requestUpdate = true;
			setVarSpecialSkill(static_cast<SpecialSkills_t>(ss), imbuement->specialSkills[ss]);
		}
	}
...
void Player::setEffectLowBlow(bool newvalue)
{
	...
	if (newvalue) {
		setVarSpecialSkill(SPECIALSKILL_CRITICALHITCHANCE, 3);
	}
	else {
		setVarSpecialSkill(SPECIALSKILL_CRITICALHITCHANCE, -3);
	}
...
void Player::setAutolootItem(uint16_t itemId, bool isLogin /* = false */)
{
	...
	// Set a fixed limit of items on auto-loot
	size_t limit = 10; //or any other value you want
